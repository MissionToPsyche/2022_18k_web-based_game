...
public class TextureAtlasSlicer : EditorWindow {
    ...
    private void UseSelectedTexture() {
        ...
        if (selectedTexture != null) {
            ...
            if (importer) {
                ...
                var fullFilePath = Path.GetFullPath(Path.Combine(assetPath, @"..\..\spriteSheets.xml"));
                var xmlPath = Path.GetRelativePath(rootPath, fullFilePath).Replace("\\", "/");
    ...
    private void ParseXML() {
        try {
            ...
            Debug.Log($"scripts : {root.ChildNodes.Cast<XmlNode>().Where(childNode => childNode.Name == "sprite").Count()}");
            subTextures = root.ChildNodes[0].ChildNodes
                              .Cast<XmlNode>()
                              .Where(childNode => childNode.Name == "sprite")
                              .Select(childNode => new SubTexture {
                                  width = Convert.ToInt32(childNode.Attributes["rect"].Value.Split(",")[2]),
                                  height = Convert.ToInt32(childNode.Attributes["rect"].Value.Split(",")[3]),
                                  x = Convert.ToInt32(childNode.Attributes["rect"].Value.Split(",")[0]),
                                  y = Convert.ToInt32(childNode.Attributes["rect"].Value.Split(",")[1]),
